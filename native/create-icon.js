...
for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) {
  ...
  const globe = Math.abs(r - 78) < 5 || (r < 78 && (Math.abs(dy) < 4 || Math.abs(Math.hypot(dx * 2.2, dy) - 78) < 5));
  const i = y * (size * 4 + 1) + 1 + x * 4;
  const color = globe ? [233, 249, 238, 255] : r < 120 ? [33, 100, 80, 255] : [0, 0, 0, 0];
  color.forEach((value, offset) => { pixels[i + offset] = value; });
}
const header = Buffer.alloc(13); header.writeUInt32BE(size); header.writeUInt32BE(size, 4); header[8] = 8; header[9] = 6;
writeFileSync(new URL('./icon.png', import.meta.url), Buffer.concat([Buffer.from([137,80,78,71,13,10,26,10]), chunk('IHDR', header), chunk('IDAT', deflateSync(pixels)), chunk('IEND', Buffer.alloc(0))]));
