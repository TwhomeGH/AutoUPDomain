...
「網域狀態」右上可切換三種顯示方式，偏好同樣只留在目前瀏覽器：**截斷**（預設，只顯示前 12 個字）、**完整**（完整網域）與**隱藏**（直播模式，以固定編號 `網域 #1`、`#2`… 取代，並同步遮蔽執行控制台與下載日誌中的完整網域）。剩餘天數依門檻變色（小於續期門檻為琥珀、7 天內為紅色），檢查結果以狀態標籤呈現。
...
Web UI 控制台每秒更新實際查詢、略過、續期確認、重試與通知流程，保留最近 200 筆；檢查尚未結束時也能看到進度。可篩選警告／錯誤、停止畫面更新、自動捲動、下載目前緩存的日誌，或手動清除（會同步清空保存的紀錄，需二次確認）。暫停控制台更新不會停止排程。憑證會遮蔽；同一份流程紀錄亦輸出到啟動終端。最近紀錄於任務完成、儲存設定及正常退出時保存。
