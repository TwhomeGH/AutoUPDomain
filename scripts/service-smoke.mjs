...
async function launch() {
  const child = spawn(process.execPath, ['src/main.js', '--no-open', '--no-tray', '--ignore-env'], {
    windowsHide: true,
    env: { ...process.env, AUTOUPDOMAIN_DATA_DIR: directory, AUTOUPDOMAIN_PORT: '0' },
  });
  const url = await new Promise((resolve, reject) => {
    ...
    child.stdout.on('data', (chunk) => {
      text += chunk;
      const match = text.match(/http:\/\/127\.0\.0\.1:\d+\/#[a-f0-9]+/);
      if (match) {
        clearTimeout(timer);
        resolve(match[0]);
      }
  ...
  return {
    ...
    async request(endpoint, body) {
      const response = await fetch(parsed.origin + '/api/' + endpoint, {
...
try {
  await instance.request('settings', {
    paused: true,
    apiKey: 'isolated-test-key',
    apiSecret: 'isolated-test-secret',
    intervalHours: 12,
    notificationMode: 'errors-only',
  });
  ...
  if (
    ...
    !state.hasApiKey ||
    !state.hasApiSecret ||
    state.settings.notificationMode !== 'errors-only' ||
    state.settings.intervalHours !== 12
  )
    throw new Error('Settings not restored');
  if (state.lastRunAt !== null) throw new Error('Paused instance unexpectedly called API');
  console.log(
    'PASS pure Node service starts, encrypts credentials, restores settings after restart, keeps paused without calling API',
  );
} finally {
  await instance.stop();
}
