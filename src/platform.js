...
export function dataDirectory() {
  return (
    ...
    (process.platform === 'win32'
      ? path.join(process.env.APPDATA || os.homedir(), 'AutoUPDomain')
      : path.join(
...
/** 使用參數陣列啟動原生程式，不經 shell；敏感內容只透過 stdin 傳入。 */
export function command(file, args, input, timeout = 15000) {
  return new Promise((resolve, reject) => {
    const child = execFile(
      ...
      (error, stdout) =>
        error
          ? reject(new Error('系統工具執行失敗，請確認所需服務與權限'))
          : resolve(stdout.trim()),
    );
...
/** Windows 交給 DPAPI；Linux 將 AES 金鑰留在登入金鑰圈，磁碟只保留密文。 */
export function createVault(directory) {
  ...
  async function linuxKey(create) {
    ...
    let stored;
    try {
      stored = await command('secret-tool', ['lookup', 'application', appId, 'profile', directory]);
    } catch {
      if (!create) throw new Error('無法讀取系統金鑰圈；請確認 libsecret 與登入金鑰圈已解鎖');
    }
    if (!stored && create) {
      stored = randomBytes(32).toString('base64');
      await command(
        'secret-tool',
        ['store', '--label=AutoUPDomain', 'application', appId, 'profile', directory],
        stored,
      );
    }
    ...
    if (key.length !== 32) {
      key = null;
      throw new Error('系統金鑰圈無效，無法解密憑證');
    }
    return key;
  }
  const dpapi = (action, value) =>
    command(
      'powershell.exe',
      [
        '-NoProfile',
        ...
        'Bypass',
        '-File',
        path.join(projectRoot, 'native', 'secrets.ps1'),
      ],
  ...
  return {
    async encrypt(value) {
      if (process.platform === 'win32')
        return { type: 'dpapi', value: await dpapi('encrypt', value) };
    ...
    async decrypt(record) {
      if (process.platform === 'win32' && record.type === 'dpapi')
        return dpapi('decrypt', record.value);
      if (process.platform === 'linux' && record.type === 'libsecret-aes') {
        const decipher = createDecipheriv(
          'aes-256-gcm',
          await linuxKey(false),
...
/** 只負責作業系統通知，不決定何時通知；排程與手動通知規則集中在核心。 */
export function createSystemNotifier() {
  ...
  return async (title, body) => {
    ...
    const binary = path.join(
      ...
      'vendor',
      'snoreToast',
      `snoretoast-${process.arch === 'ia32' ? 'x86' : 'x64'}.exe`,
    );
    if (!registered) {
      const shortcut = path.join(
        ...
        'Start Menu',
        'Programs',
        'AutoUPDomain Notifications.lnk',
      );
      try {
        await access(shortcut);
      } catch {
        await command(binary, ['-install', shortcut, binary, appId]);
      }
      registered = true;
    }
    // SnoreToast 的 1/2/3 表示隱藏、關閉或到時結束，不能視為呼叫失敗。
    // 這些回傳碼也無法證明使用者真的看到了通知。
    await new Promise((resolve, reject) => {
      execFile(
        binary,
        ['-appID', appId, '-t', title, '-m', body, '-p', icon, '-silent'],
