...
test('OS socket prevents duplicate instances and releases on close', async t => {
  ...
  assert.equal(second.existing, true); assert.match(second.url, /mock-token/);
  await first.close();
  const third = await acquireInstance(directory, () => '');
  assert.equal(third.existing, false); await third.close();
});
test('Windows DPAPI encrypts and reloads credentials without plaintext disk storage', { skip: process.platform !== 'win32' }, async () => {
