...
test('day schedules survive reload and wait until exactly due', async () => {
  ...
  const restored = new DomainService({
    config: JSON.parse(JSON.stringify(first.config)),
    api,
    state: saved,
    now: () => now,
  });
...
test('old hourly settings migrate and invalid day settings are rejected', () => {
  ...
  for (const config of [
    { intervalHours: 2184 },
    { intervalHours: 12, intervalUnit: 'days' },
    { intervalUnit: 'months' },
  ])
...
test('live console exposes progress before completion and redacts secrets', async () => {
  ...
  const service = new DomainService({
    config: {
      ...
      barkUrl: 'https://example.test/private-bark',
    },
    api: {
      list: () =>
        new Promise((resolve) => {
  ...
  assert.ok(!output.includes('private-bark'));
...
test('API query retries are reported as console progress', async () => {
  ...
  await api.list({ apiKey: 'mock', apiSecret: 'mock' }, (level, message) =>
